<div class="modal fade" id="taskDetailModal-{{ $taskStat['task_id'] }}" tabindex="-1"
                    ...
                    <div class="row g-4">
                                ...
                                @foreach ($team as $teamMember)
                                    @if(!empty($teamMember->profile_picture))
                                        ...
                                        <div class="d-flex align-items-center bg-white rounded-pill px-3 py-1 shadow-xs"
                                            ...
                                            <img src="../assets/images/profile_picture/{{$teamMember->profile_picture}}" class="rounded-circle me-2"
                                                width="20" height="20" style="object-fit: cover;">
                                            <span class="text-dark fs-6 fw-medium">{{ $teamMember->employee_name }} </span>
                                        </div>
                                    @endif
                                @endforeach
                        ...
                        <div class="col-12">
                            ...
                            <div class="border rounded p-3 bg-white border-gray-200">
                                ...
                                <!-- Link Attachments -->
                                @php
                                    $linkAttachments = $taskMedias->where('category', 'link')->where('task_id', $taskStat['task_id'])->groupBy('task_id');
                                @endphp
                                ...
                                <!-- Link Attachments -->
                                @php
                                    $voiceNotes = $taskMedias->where('category', 'voice_note')->where('task_id', $taskStat['task_id'])->groupBy('task_id');
                                @endphp
